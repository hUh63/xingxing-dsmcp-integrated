...
private fun clientConfig(url: String, settings: SettingsStore): String {
    ...
    val server = JSONObject()
        .put("command", "npx")
        .put("args", args)
    return prettyJson(JSONObject().put("mcpServers", JSONObject().put("niehe", server)))
}
...
private fun openInBrowser(context: Context, url: String) {
    try {
        ...
        AppLog.w("Failed to open browser: ${e.message}")
        Toast.makeText(context, if (url.isBlank()) "No service URL available" else "Failed to open browser", Toast.LENGTH_SHORT).show()
    }
}
